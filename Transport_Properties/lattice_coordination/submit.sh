...
################################################################################
# Batch Simulation Script: Coordination Number Measurement (Single Parameters)
################################################################################
#
# This script orchestrates multiple simulations to measure coordination number
# evolution with FIXED HT and packing fraction values.
#
# Workflow:
#   1. Setup CUDA environment
#   2. Validate command-line arguments
#   3. Parse parameters (LATTICE_TYPE, LX, PT, HT, PACKING_FRACTION)
...
echo -e "${MAGENTA}${BOLD}Coordination Number Measurement - Single Simulation${NC}"
echo -e "${MAGENTA}${BOLD}============================================================================${NC}\n"
...
if [[ "$LATTICE" != "SQUARE_MOORE" && "$LATTICE" != "TRIANGULAR" ]]; then
    ...
    echo -e "${YELLOW}[USAGE]${NC} ./submit.sh [LATTICE_TYPE] [LX] [PT] [HT] [PACKING_FRACTION]"
    echo -e "${YELLOW}[EXAMPLE]${NC} ./submit.sh TRIANGULAR 512 5.0 64 0.128"
    exit 1
fi
echo -e "${GREEN}[OK]${NC} Lattice type: ${BOLD}${LATTICE}${NC}"

# ========================================================================
# Parameter 2: Linear lattice size (default: 512)
# ========================================================================
LX=${2:-512}
...
# ========================================================================
# Parameter 4: Healing time (default: 64)
# ========================================================================
HT=${4:-64}
...
run_simulation() {
    ...
    echo -e "${BLUE}[INFO]${NC} Compiling simulation..."
    ${NVCC} ${ARCH_FLAGS} ${CFLAGS} ${SRC} ${LDFLAGS} 2>&1 | grep -E "error|warning" || echo -e "${GREEN}[OK]${NC} Compilation successful"
    ...
    echo -e "${BLUE}[INFO]${NC} Executing simulation..."
    ./a.out > "$output_file" 2>&1
    
    if [ $? -ne 0 ]; then
        echo -e "${RED}[ERROR]${NC} Simulation execution failed"
        rm -f "$output_file"
        return 1
    fi
    
    echo -e "${GREEN}[OK]${NC} Simulation completed"
    
    # Move data file to realization directory
...
echo -e "  ${CYAN}Persistence Time (PT):${NC}   ${BOLD}${PT}${NC}"
echo -e "  ${CYAN}Healing Time (HT):${NC}       ${BOLD}${HT}${NC}"
...
for realization in $(seq 1 $NUM_REALIZATIONS); do
    ...
    run_simulation $PACKING_FRACTION $HT $realization
    
    if [ $? -eq 0 ]; then
        total_completed=$((total_completed + 1))
    else
        echo -e "${RED}[SKIP]${NC} HT=${HT}, PACK=${PACKING_FRACTION}"
        continue
    fi
    ...
    echo -e "  ${CYAN}Output file:${NC} ${BOLD}coordination_${timestamp}_L_${LX}_PACK_${pack_fmt}_PT_${pt_fmt}_HT_${ht_fmt}_R_${realization}.dat${NC}\n"
done
