...
PT_VALUES=(1 16 32 64 128 256 512)
...
for realization in $(seq 1 $NUM_REALIZATIONS); do
    ...
    ht_fmt=$(printf "%.6f" "$HT")
    mkdir -p "$REALIZATION_DIR/COORDINATION_HT_${ht_fmt}" || { echo "Failed to create directory"; exit 1; }
    mv *.dat "$REALIZATION_DIR/COORDINATION_HT_${ht_fmt}/" || { echo "Failed to move files"; exit 1; }
