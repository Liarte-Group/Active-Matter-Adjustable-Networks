...
run_simulation() {
    ...
    # Calculate P_PERST and P_REGEN from PT (global) and HT
    # P_PERST = 1 - (1/PT)
    ...
    ./a.out > "coordination_${timestamp}_L_${LX}_PACK_${pack_fmt}_PT_${pt_fmt}_HT_${ht_fmt}_R_${realization}.dat" || { echo "Simulation failed"; exit 1; }
}
...
for realization in $(seq 1 $NUM_REALIZATIONS); do
    ...
    pt_fmt=$(printf "%.6f" "$PT")
    mkdir -p "$REALIZATION_DIR/COORDINATION_PT_${pt_fmt}" || { echo "Failed to create directory"; exit 1; }
    mv *.dat "$REALIZATION_DIR/COORDINATION_PT_${pt_fmt}/" || { echo "Failed to move files"; exit 1; }
