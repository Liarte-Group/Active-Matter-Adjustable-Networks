...
__global__ void updateParticles(int *site, int *neighbor, int *bond, int *index, int *direction, int *stopReason,
 int nParticles, int dim, int z, curandState *states) {
   ...
   // Check if movement is allowed through the link
    int linkOK = bond[z * oldIdx + dir];
    int moved  = 0;
    int siteOK = 0;

    // Non-invasive check of the occupancy status of the target site, regardless of link state
    int siteOccupied = (site[newIdx] == 1);               // read-only access, no modification
    ...
    if (linkOK) {
        ...
        if (atomicCAS(&site[newIdx], expected, 1) == 0) {
            ...
            siteOK = 1;  // Occupation failed: target site was not empty
        }
    } else {
        if (siteOccupied) {
            siteOK = 1;  // Occupation failed: target site was not empty
    ...
    // Classification of the stopping reason
    if (moved) {
        stopReason[idx] = 0;            // Movement was successful
    } else if (!linkOK && siteOK) {
        stopReason[idx] = 3;            // Blocked due to both link and site occupancy
    } else if (!linkOK) {
        stopReason[idx] = 1;            // Blocked due to link being inactive
    } else if (siteOK) {
        stopReason[idx] = 2;            // Blocked due to site being occupied
    } else {
        stopReason[idx] = 99;           // Debug: unexpected case
    }
...
// CUDA kernel to count the number of particles stopped for each reason
__global__ void reduceStopCauses(const int* stopReason, int* counts, int nParticles) {
    // Compute global thread index
    int idx = blockIdx.x * blockDim.x + threadIdx.x;

    // Check that the index is within bounds
    if (idx < nParticles) {
        // Retrieve the stop reason for this particle
        int cause = stopReason[idx];

        // Accumulate the count for valid causes (0 to 3)
        if (cause >= 0 && cause <= 3) {
            atomicAdd(&counts[cause], 1);
        }
        // If cause == 99 (or others), it is ignored here
    }
}