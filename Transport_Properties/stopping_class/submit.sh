...
# class distribution across varying network parameters.
#
# Workflow:
...
echo -e "${MAGENTA}${BOLD}Stopping Class Analysis - Single Simulation${NC}"
echo -e "${MAGENTA}${BOLD}============================================================================${NC}\n"
...
if [[ "$LATTICE" != "SQUARE_MOORE" && "$LATTICE" != "TRIANGULAR" ]]; then
    ...
    echo -e "${YELLOW}[USAGE]${NC} ./stopping_class.sh [LATTICE_TYPE] [LX] [HT] [PT] [PACKING_FRACTION]"
    echo -e "${YELLOW}[EXAMPLE]${NC} ./stopping_class.sh TRIANGULAR 32 40.0 256 0.064"
    exit 1
fi
...
# ========================================================================
# Parameter 5: Packing fraction (default: 0.064)
# ========================================================================
PACKING_FRACTION=${5:-0.064}
...
run_simulation() {
    ...
    mv "$output_file" "$REALIZATION_DIR/STOPPING_CLASS_HT_${ht_fmt}/" 2>/dev/null
...
for realization in $(seq 1 $NUM_REALIZATIONS); do
    ...
    echo -e "  ${CYAN}Output file:${NC} ${BOLD}stoppingClass_${timestamp}_L_${LX}_PACK_${pack_fmt}_PT_${pt_fmt}_HT_${ht_fmt}_R_${realization}.dat${NC}\n"
done
