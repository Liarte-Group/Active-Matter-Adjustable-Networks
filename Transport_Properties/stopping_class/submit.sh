...
run_simulation() {
    ...
    ./a.out > "stoppingClass_${timestamp}_L_${LX}_PACK_${pack_fmt}_PT_${pt_fmt}_HT_${ht_fmt}_R_${realization}.dat" || { echo "Simulation failed"; exit 1; }
}
...
for realization in $(seq 1 $NUM_REALIZATIONS); do
    ...
    ht_fmt=$(printf "%.6f" "$HT")
    mkdir -p "$REALIZATION_DIR/STOPPING_CLASS_${ht_fmt}" || { echo "Failed to create directory"; exit 1; }
    mv *.dat "$REALIZATION_DIR/STOPPING_CLASS_${ht_fmt}/" || { echo "Failed to move files"; exit 1; }
